...
additionalTable1 <- function(connectionDetails = connectionDetails,
  ...
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "NSAID")
    ...
    for(endDay in c(0,-1)){
      ...
      tryCatch({
        comparativeCharacterization(connectionDetails = connectionDetails,
                                    ...
                                    comparatorCohortId = comparatorId,#7,
                                    ...
                                    fileName = file.path(exportFolder, sprintf("base_char_t%s_c%s_until%s.csv",targetId, comparatorId, endDay)))
      },
      error = function(e) {
        ParallelLogger::logTrace(paste0(sprintf("Generating table 1 for cohort ID %s is failed. The error message:", targetId), e))},
      finally = {
        ParallelLogger::logTrace('Done.')})
      
      stratPopPaths <- list.files(file.path(outputFolder,"cmOutput"),sprintf("StratPop_.*t%s_c%s.*.rds" , targetId, comparatorId))
      for(popPath in stratPopPaths){
        stratPop <- readRDS(file.path(outputFolder,"cmOutput", popPath))
        tryCatch({
          ...
          comparativeCharacterization(connectionDetails = connectionDetails,
                                      ...
                                      studyPopulationSetting = NULL,
                                      stratPop = stratPop,
                                      fileName = file.path(exportFolder, sprintf("base_char_%s_until%s.csv",gsub("\\.rds","",popPath), endDay)))
        },
        error = function(e) {
          ParallelLogger::logTrace(paste0(sprintf("Generating table 1 for cohort ID %s is failed. The error message:", targetId), e))},
        finally = {
...
comparativeCharacterization <- function(connectionDetails,
  ...
  if(is.null(outcomeCohortIds)){
    ...
    if (!is.null(stratPop)){
      targetCovData <- mapCovariates(targetCovData,
                                     stratPop,
                                     treatment = 1)
    }
    ...
    if(!is.null(comparatorCohortId)){
      ...
      if (!is.null(stratPop)){
        comparatorCovData <- mapCovariates(comparatorCovData,
                                           stratPop,
                                           treatment = 0)
      }
...
mapCovariates <- function(covData,
                          stratPop,
                          treatment){
  covariates = covData$covariates
  covariateRef = covData$covariateRef
  stratPop <- stratPop[stratPop$treatment==treatment,]
  ...
  idx <- ffbase::ffmatch(x = covariates$rowId, table = ff::as.ff(stratPop$subjectId))
  idx <- ffbase::ffwhich(idx, !is.na(idx))
  covariates <- covariates[idx, ]
  covData$metaData$populationSize <- nrow(stratPop)
  
  covariateData = list(covariates = covariates,
                       covariateRef = covariateRef,
                       timeRef = covData$timeRef,
                       analysisRef = covData$analysisRef,
                       metaData = covData$metaData)
