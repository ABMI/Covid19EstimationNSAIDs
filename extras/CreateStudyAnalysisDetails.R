...
createAnalysesDetails <- function(workFolder) {
  makeCovariateIdsToInclude <- function(includeIndexYear = FALSE) {
    ageGroupIds <- unique(floor(c(18:110)/5) * 1000 + 3)
  ...
  htnIngredientConceptIds <- c(1103374,
                               1103314,
                               1102917,
                               19042155,
                               19041220,
  ...
  covarSettingsWithoutHtnMeds <- FeatureExtraction::createDefaultCovariateSettings(excludedCovariateConceptIds = htnIngredientConceptIds,
  ...
  getDbCmDataArgsWithHtnMeds <- CohortMethod::createGetDbCohortMethodDataArgs(firstExposureOnly = firstExposureOnly,
                                                                              studyStartDate = studyStartDate,
                                                                              removeDuplicateSubjects = "remove all",
                                                                              excludeDrugsFromCovariates = FALSE,
                                                                              covariateSettings = covarSettingsWithHtnMeds)

  getDbCmDataArgsWithoutHtnMeds <- CohortMethod::createGetDbCohortMethodDataArgs(firstExposureOnly = firstExposureOnly,
                                                                                 studyStartDate = studyStartDate,
                                                                                 removeDuplicateSubjects = "remove all",
                                                                                 excludeDrugsFromCovariates = FALSE,
  ...
  createLargeScalePsArgs <- CohortMethod::createCreatePsArgs(stopOnError = FALSE,
                                                             prior = Cyclops::createPrior(priorType = "laplace",
                                                                                                               useCrossValidation = TRUE))

  createLargeScalePsArgsNoCv <- CohortMethod::createCreatePsArgs(stopOnError = FALSE,
                                                                 prior = Cyclops::createPrior(priorType = "laplace",
                                                                                                                   variance = fixedPsVariance,
                                                                                                                   useCrossValidation = FALSE))

  fitUnadjustedOutcomeModelArgs <- CohortMethod::createFitOutcomeModelArgs(modelType = "logistic",
                                                                           useCovariates = FALSE,
                                                                           stratified = FALSE)

  fitAdjustedOutcomeModelArgs <- CohortMethod::createFitOutcomeModelArgs(modelType = "logistic",
                                                                         useCovariates = TRUE,
                                                                         includeCovariateIds = makeCovariateIdsToInclude(),
                                                                         stratified = FALSE,
                                                                         prior = Cyclops::createPrior(priorType = "normal",
  ...
  fitPsOutcomeModelArgsConditioned <- CohortMethod::createFitOutcomeModelArgs(modelType = "logistic",
                                                                              useCovariates = FALSE,
                                                                              stratified = TRUE)
  fitPsOutcomeModelArgsUnConditioned <- CohortMethod::createFitOutcomeModelArgs(modelType = "logistic",
                                                                                useCovariates = FALSE,
                                                                                stratified = FALSE)

  stratifyByPsArgs <- CohortMethod::createStratifyByPsArgs(numberOfStrata = 5)

  matchByPsArgs <- CohortMethod::createMatchOnPsArgs(maxRatio = 1  # TODO Allow for multiple matches
)

  matchByPsArgsVariable <- CohortMethod::createMatchOnPsArgs(maxRatio = 100  # TODO Allow for multiple matches
)
  ...
  cmAnalysisList <- list(cmAnalysis1,
                         ...
                         cmAnalysis3,
                         cmAnalysis4,
                         cmAnalysis5)  #,cmAnalysis7,cmAnalysis8)
